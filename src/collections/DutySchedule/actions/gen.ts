...
import { calendarDaysBetween, toLocalDateString } from '@/helpers/dutyScheduler'
...
export const generate = async (year: number, month: number) => {
  ...
  // 2) NÖBET TÜRLERİ ÇEK — yıl filtresi WHERE içinde (DB seviyesinde)
  ...
  for (const dt of dutyTypes) {
    ...
    console.log(`  Cron ifadeleri:`, dtAny.cronSchedules)
    const dates = getDutyDatesForMonth(dtAny.cronSchedules, year, month)
  ...
  // 👇 Cooldown sadece TEK SEFER gevşetilebilir (session-scoped flag)
  ...
  for (let i = 0; i < sortedDutyDays.length; i++) {
    ...
    // 👇 Yeniden kullanılabilir cooldown helper'ı (strict veya relaxed gap ile çalışır)
    const cooldownConflict = (personId: number, gapDays: number) => {
      const assignedDates = personAssignedDates.get(personId) || new Set<string>()
      const dutyMidnight = new Date(
        duty.date.getFullYear(),
      ...
      const checkStart = new Date(dutyMidnight)

      checkStart.setDate(checkStart.getDate() - gapDays)

      const checkEnd = new Date(dutyMidnight)

      checkEnd.setDate(checkEnd.getDate() + gapDays)

      if (personId === 4) {
        console.log(
          '🪲 Rüstem check — duty.date:',
          duty.date?.toISOString(),

          '| dutyMidnight:',
          dutyMidnight.toISOString(),
      ...
      for (const ds of assignedDates) {
        ...
        if (ad.getTime() >= checkStart.getTime() && ad.getTime() <= checkEnd.getTime()) {
          const diff = calendarDaysBetween(ad, dutyMidnight) // ← normalle

          if (diff <= gapDays) return { conflict: true, conflictingDate: ds, diff }
        }
      }
    ...
    for (const person of sorted) {
      if (!normalAssigned) {
        ...
        // 👇 Cooldown kontrolü (strict — duty.cooldownDays)
        const cd = cooldownConflict(person.id, duty.cooldownDays)
        if (cd.conflict) {
          addLog(
            'warning',
            `⏭️ ${person.fullName} cooldown (${cd.diff}/${duty.cooldownDays}) -> ${cd.conflictingDate}, atlandı.`,
            '429',
            ['islem', dutyDateStr, groupKey],
          )
          continue
    ...
    // 👇 Fallback — gevşek cooldown (cooldown − 1), sadece TEK SEFER (session başına)
    if (!normalAssigned && duty.cooldownDays > 0) {
      const relaxedDays = duty.cooldownDays - 1

      addLog(
        'warning',

        `⚠️ Sıkışma: gevşek cooldown (${relaxedDays} gün) deneniyor...`,
      ...
      for (const person of sorted) {
        ...
        const cd = cooldownConflict(person.id, relaxedDays)
        if (cd.conflict) continue
        ...
        addLog(
          'warning',
          `⚠️ ${person.fullName} gevşek cooldown (${relaxedDays} gün) ile atandı.`,
          '200',
          ['islem', dutyDateStr, groupKey],
        )
        break

...
export const validateSingleUnofficialDuty: CollectionBeforeValidateHook = async ({
  ...
  // ── Personel ID'sini al ──
  const rawPersonnel = data?.personnel ?? originalDoc?.personnel
  const personnelId = typeof rawPersonnel === 'object' ? rawPersonnel?.id : rawPersonnel

  if (!personnelId) return

  // ── Personeli çek, GERÇEK grubunu bul ──
  const personnel = await req.payload.findByID({
    collection: 'personnel',
    id: personnelId,
  })

  const rawGroup = personnel?.group
  const groupId = typeof rawGroup === 'object' ? rawGroup?.id : rawGroup

  if (!groupId) return

  // ── Sorguda duty_schedule.group DEĞİL, personnel.group üzerinden kontrol ──
  const whereCondition: any = {
    and: [
      { dutyDate: { equals: data.dutyDate } },
      { isOffical: { equals: false } },
      { 'personnel.group': { equals: groupId } }, // ← personelin grubu
    ],
  }
  ...
  if (existing.totalDocs > 0) {
    ...
    const gName = typeof found.group === 'object' ? found.group?.name : '?'
    throw new Error(
      `"${gName}" grubundaki bir personel için bu tarihte zaten resmi olmayan bir nöbet kaydı mevcut (ID: ${found.id}).`,
    )
  }
}

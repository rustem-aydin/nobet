...
export function rangeText(view: TCalendarView, date: Date): string {
  ...
  switch (view) {
    ...
    case 'week':
      ...
      break
    case 'day':
      return format(date, FORMAT_STRING, { locale: tr })
    case 'year':
  ...
  return `${format(start, FORMAT_STRING, { locale: tr })} - ${format(end, FORMAT_STRING, { locale: tr })}`
}
...
export function getCalendarCells(selectedDate: Date): ICalendarCell[] {
  ...
  const prevMonthCells = Array.from({ length: startOffset }, (_, i) => {
    const date = new Date(year, month - 1, daysInPrevMonth - startOffset + i + 1)
    return {
      day: daysInPrevMonth - startOffset + i + 1,
      currentMonth: false,
      date,
      dayOfWeek: i, // 0, 1, 2, 3, 4, 5, 6
    }
  })

  const currentMonthCells = Array.from({ length: daysInMonth }, (_, i) => {
    const date = new Date(year, month, i + 1)
    const dayIndex = (startOffset + i) % 7
    return {
      day: i + 1,
      currentMonth: true,
      date,
